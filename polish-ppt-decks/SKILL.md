...
11. Run `scripts/validate_illustration_assets.mjs <task-workspace>/assets` before placement when illustrations were cropped from asset sheets. Reject detached edge fragments; if a clean copy is required, write it to a new asset directory with `--output-dir` and keep the original source unchanged.
...
- Prefer subtraction. A cover normally needs one title hierarchy, one subtitle, one speaker identity, and one hero visual.
...
- When full-slide coverage was requested, every slide has at least one topic-specific illustration and every semantic card or node that reads as a distinct concept has its own illustration.
...
- Sheet-cropped PNGs pass `scripts/validate_illustration_assets.mjs`; no detached edge fragment is hidden with a slide-level mask.
